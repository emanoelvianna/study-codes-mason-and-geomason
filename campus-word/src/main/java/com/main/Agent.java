...
public class Agent implements Steppable {
  ...
  public Agent(CampusWorld state) {
    location = new MasonGeometry(fact.createPoint(new Coordinate(10, 10))); // magic
                                                                            // numbers

    location.isMovable = true;

    // Find the first line segment and set our position over the start
    // coordinate.
   ...
   *          defining new route
   * @param start
   *          true if agent at start of line else agent placed at end
   */
